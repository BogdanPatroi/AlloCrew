...
const Announcement = ({
  title, location, description, picture, voluntary, id, dateEnd, dateStart, user, sendingMessage, createdAt, userId, redirect, deleteD,
}) => {
  if (redirect === 'tchat') {
      ...
      <div className="announcement__pos1">
        ...
        {/* Affichage conditionnel du bouton */}
        {
          userId != user.id && (
            ...
            <form
              onSubmit={(e) => {
                e.preventDefault(); sendingMessage({ user_id: user.id, announcement_id: id });
              }}
              method="post"
            >
        ...
        <br />
        <img alt="" className="arrow" src={arrow} />
      </div>
    </div>
  );
};
